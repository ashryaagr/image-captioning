...
#         printf("feat after linear", y.shape)
        ...
        embed = torch.cat((y.unsqueeze(1),embed), dim =1 ) 
#         h_c = (feat, feat)
#         printf("feat after unsqueezing", feat.shape) # [6, 1, 300]
        
#         embed = torch.cat((feat, embed), dim = 1)

#         packedEmbed = pack_padded_sequence(embed, lengths, batch_first=True, enforce_sorted=False)
#         printf("packedEmbed", type(packedEmbed))
        
#         printf(embed.shape, len(h_c.shape))
...
#                 printf("im inside")
                hiddens , states = self.lstm(y,states)
